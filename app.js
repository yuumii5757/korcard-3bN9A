const App = {
    ...
    loadSettings() {
        const saved = localStorage.getItem('korcard_settings');
        if (saved) {
    ...
    saveSettings() {
        localStorage.setItem('korcard_settings', JSON.stringify(this.settings));
        this.applyTheme();
    },
    ...
    renderManage(params) {
        ...
        const renderList = () => {
            ...
            filtered.forEach(c => {
                ...
                el.innerHTML = `
                    ...
                    <div class="list-item-sub kor-font text-primary flex align-center gap-2">
                        ${c.target_text} 
        ...
        document.getElementById('btnExport').addEventListener('click', () => {
            ...
            downloadAnchorNode.setAttribute("download", "korcard_backup.json");
        ...
        const modalHtml = `
            <div class="modal-overlay active" id="cardModal">
                <div class="modal-content">
                    ...
                    <div class="form-group mb-3">
                        <label class="block mb-1 text-sm font-bold">韓国語 (正解) <span class="text-danger">*</span></label>
                        <input type="text" id="mTarget" class="form-control kor-font text-lg" placeholder="例: 안녕하세요" value="${c.target_text}">
                    </div>

                    <div class="form-group mb-3">
                        <label class="block mb-1 text-sm font-bold">発音記号</label>
                        <input type="text" id="mPronunciation" class="form-control" placeholder="例: annyeonghaseyo" value="${c.pronunciation || ''}">
                    </div>
    ...
    renderQuiz(params) {
        ...
        document.getElementById('btnShowAnswer').addEventListener('click', () => {
            ...
            if (this.data.currentQuiz.mode === 'jp-kor') {
                ...
                tarEl.classList.remove('kor-font', 'text-primary');
                tarEl.classList.add('text-xl');

                // Show pronunciation just below the Kor text (which is the prompt)
                if (c.pronunciation) {
                    const promptHintEl = document.getElementById('qPromptHint');
    ...
    updateQuizUI() {
        ...
        if (cq.mode === 'jp-kor') {
            ...
            answerLabelEl.textContent = '韓国語';
        } else {
            promptLabelEl.textContent = '韓国語';
            promptEl.textContent = c.target_text;
            promptEl.className = 'kor-font text-2xl text-primary font-bold';
            answerLabelEl.textContent = '日本語';

            // Auto TTS when showing Kor prompt!
            this.speakText(c.target_text);
        }
    ...
    renderResult() {
        ...
        if (cq.wrongCards.length > 0) {
            ...
            cq.wrongCards.forEach(c => {
                ...
                el.innerHTML = `
                    <div class="list-item-title">${c.native_text}</div>
                    <div class="list-item-sub kor-font text-primary">${c.target_text} <span class="text-muted ml-2">${c.pronunciation || ''}</span></div>
                `;
    ...
    renderSettings() {
        ...
        const populateVoices = () => {
            ...
            // Filter primarily for Korean (ko-KR), but keep others as fallback
            const korVoices = voices.filter(v => v.lang.startsWith('ko-KR') || v.lang.startsWith('ko'));
            const showVoices = korVoices.length > 0 ? korVoices : voices;
        ...
        document.getElementById('btnTestVoice').addEventListener('click', () => {
            this.speakText('안녕하세요, 만나서 반갑습니다');
        });
